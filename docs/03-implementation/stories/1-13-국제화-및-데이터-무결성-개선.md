# Story 1.13: 국제화 및 데이터 무결성 개선

**Status:** ✅ DONE

**Priority:** 🟢 LOW

---

## User Story

**As a** 한국어 사용자,
**I want** 모든 에러 메시지와 시스템 메시지가 한국어로 표시되고 데이터 무결성이 보장되어,
**So that** 일관된 사용자 경험과 신뢰할 수 있는 시스템을 이용할 수 있다.

---

## Context

**Code Review 발견 사항 (Story 1.10):**

### Issue #1: 영어 에러 메시지 (HIGH)
**문제:**
```typescript
// src/server/api/routers/team.ts
throw new TRPCError({
  code: "FORBIDDEN",
  message: "You do not have access to this team",  // ❌ English
});

throw new TRPCError({
  code: "NOT_FOUND",
  message: "Team not found",  // ❌ English
});
```

```typescript
// src/server/api/routers/project.ts
throw new TRPCError({
  code: "FORBIDDEN",
  message: "You do not have permission to access this project",  // ❌ English
});

throw new TRPCError({
  code: "BAD_REQUEST",
  message: "Reason is required",  // ❌ English
});
```

**문제점:**
- UI는 한국어인데 에러 메시지는 영어로 표시
- 일관성 없는 사용자 경험
- 한국어 사용자의 이해도 저하

**해결 방법:**
- 모든 backend 에러 메시지를 한국어로 번역
- 에러 메시지 constants 파일 생성하여 중앙 관리

### Issue #2: Audit Log Transaction Safety (MEDIUM)
**문제:**
```typescript
// src/server/api/routers/project.ts - deleteApiKey
await db.auditLog.create({
  data: {
    userId,
    actionType: "api_key_deleted",
    resourceId: input.apiKeyId,
    metadata: { reason: input.reason }
  }
});  // ⚠️ If this succeeds but delete fails, orphaned audit log

await db.apiKey.delete({
  where: { id: input.apiKeyId }
});  // ⚠️ If this fails, audit log is incorrect
```

**문제점:**
- Audit log 생성과 실제 삭제가 별도 트랜잭션
- 삭제 실패 시 audit log가 잘못된 상태로 남음
- 데이터 무결성 위반

**해결 방법:**
```typescript
await db.$transaction([
  db.auditLog.create({ ... }),
  db.apiKey.delete({ ... }),
]);
```

### Issue #3: Input Validation Length Limits (MEDIUM)
**문제:**
```typescript
// src/server/api/routers/project.ts
disableApiKey: protectedProcedure
  .input(z.object({
    apiKeyId: z.string(),
    reason: z.string().optional(),  // ⚠️ No max length
  }))
```

**문제점:**
- 사유 필드에 길이 제한 없음
- 악의적으로 긴 텍스트 입력 가능 (DoS)
- DB 저장 공간 낭비

**해결 방법:**
```typescript
reason: z.string().min(1, "사유는 필수입니다").max(500, "사유는 500자 이내로 입력해주세요")
```

### Issue #4: Inconsistent Error Handling (LOW)
**문제:**
- 일부 에러는 toast로 표시, 일부는 console.error
- 에러 로깅 표준 부재

**해결 방법:**
- 통일된 에러 처리 패턴 적용
- Sentry 등 에러 추적 시스템 통합 (선택사항)

---

## Acceptance Criteria

### 1. 한국어 에러 메시지 적용
- [ ] `src/lib/error-messages.ts` 파일 생성 (에러 메시지 constants)
- [ ] 모든 backend 에러 메시지를 한국어로 변환:
  - `team.ts`: 5개 에러 메시지
  - `project.ts`: 12개 에러 메시지
  - 기타 routers
- [ ] Zod validation 에러 메시지도 한국어로 설정
- [ ] 프론트엔드 toast 메시지 일관성 확인
- [ ] 모든 에러 시나리오 테스트하여 한국어 표시 확인

### 2. 데이터 무결성 강화 (Transactions)
- [ ] `deleteApiKey` mutation에 transaction 적용
- [ ] `disableApiKey` mutation에 transaction 적용
- [ ] `enableApiKey` mutation에 transaction 적용
- [ ] Critical operations에 transaction 패턴 적용:
  - Audit log + actual operation = atomic
  - Rollback on error
- [ ] Transaction 실패 시 명확한 에러 메시지 (한국어)

### 3. Input Validation 강화
- [ ] 모든 string 입력에 max length 설정:
  - `reason`: 500자
  - `projectName`: 100자
  - `email`: 320자 (RFC 5321)
  - `name`: 100자
- [ ] Zod schema 업데이트
- [ ] 프론트엔드에도 동일한 validation 적용 (즉각적 피드백)
- [ ] Validation 실패 시 한국어 메시지 표시

### 4. 에러 로깅 표준화
- [ ] 모든 mutation에 try-catch 추가
- [ ] 에러 발생 시 console.error로 로깅 (dev)
- [ ] Production 환경에서는 Sentry 등으로 전송 (선택사항)
- [ ] 에러 로그에 context 정보 포함 (userId, action, timestamp)

---

## Prerequisites

- Story 1.10 (프로젝트 멤버 및 API 키 관리 UI)

---

## Technical Implementation

### 1. 한국어 에러 메시지 Constants

```typescript
// src/lib/error-messages.ts
export const ERROR_MESSAGES = {
  // Team errors
  TEAM_NOT_FOUND: "팀을 찾을 수 없습니다.",
  TEAM_ACCESS_DENIED: "이 팀에 접근 권한이 없습니다.",
  TEAM_ADMIN_REQUIRED: "팀 관리자 권한이 필요합니다.",
  TEAM_MEMBER_NOT_FOUND: "팀 멤버를 찾을 수 없습니다.",
  TEAM_MEMBER_ALREADY_EXISTS: "이미 팀에 추가된 멤버입니다.",

  // Project errors
  PROJECT_NOT_FOUND: "프로젝트를 찾을 수 없습니다.",
  PROJECT_ACCESS_DENIED: "이 프로젝트에 접근 권한이 없습니다.",
  PROJECT_MEMBER_NOT_FOUND: "프로젝트 멤버를 찾을 수 없습니다.",
  PROJECT_MEMBER_ALREADY_EXISTS: "이미 프로젝트에 추가된 멤버입니다.",
  PROJECT_MEMBER_CANNOT_REMOVE_SELF: "본인을 프로젝트에서 제거할 수 없습니다.",

  // API Key errors
  API_KEY_NOT_FOUND: "API 키를 찾을 수 없습니다.",
  API_KEY_ALREADY_DISABLED: "이미 비활성화된 API 키입니다.",
  API_KEY_ALREADY_ACTIVE: "이미 활성화된 API 키입니다.",
  API_KEY_INVALID_FORMAT: "올바르지 않은 API 키 형식입니다.",
  API_KEY_DELETION_REASON_REQUIRED: "삭제 사유는 필수입니다.",
  API_KEY_DISABLE_REASON_REQUIRED: "비활성화 사유는 필수입니다.",

  // Validation errors
  VALIDATION_REASON_TOO_LONG: "사유는 500자 이내로 입력해주세요.",
  VALIDATION_NAME_TOO_LONG: "이름은 100자 이내로 입력해주세요.",
  VALIDATION_EMAIL_INVALID: "올바른 이메일 주소를 입력해주세요.",
  VALIDATION_REQUIRED_FIELD: "필수 입력 항목입니다.",

  // Generic errors
  INTERNAL_SERVER_ERROR: "서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요.",
  UNAUTHORIZED: "인증이 필요합니다.",
  FORBIDDEN: "권한이 없습니다.",
} as const;
```

**Usage:**
```typescript
// src/server/api/routers/team.ts
import { ERROR_MESSAGES } from "@/lib/error-messages";

getMembers: protectedProcedure
  .input(z.object({ teamId: z.string() }))
  .query(async ({ ctx, input }) => {
    const members = await db.teamMember.findMany({ ... });

    const currentUserMembership = members.find(m => m.userId === userId);
    if (!currentUserMembership) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: ERROR_MESSAGES.TEAM_ACCESS_DENIED,  // ✅ Korean
      });
    }

    return members;
  }),
```

### 2. Zod Validation 한국어 메시지

```typescript
// src/server/api/routers/project.ts
import { ERROR_MESSAGES } from "@/lib/error-messages";

disableApiKey: sensitiveProcedure
  .input(
    z.object({
      apiKeyId: z.string(),
      reason: z
        .string()
        .min(1, ERROR_MESSAGES.API_KEY_DISABLE_REASON_REQUIRED)
        .max(500, ERROR_MESSAGES.VALIDATION_REASON_TOO_LONG),
    })
  )
  .mutation(async ({ ctx, input }) => {
    // ... implementation
  }),

deleteApiKey: sensitiveProcedure
  .input(
    z.object({
      apiKeyId: z.string(),
      reason: z
        .string()
        .min(1, ERROR_MESSAGES.API_KEY_DELETION_REASON_REQUIRED)
        .max(500, ERROR_MESSAGES.VALIDATION_REASON_TOO_LONG),
    })
  )
  .mutation(async ({ ctx, input }) => {
    // ... implementation with transaction
  }),
```

### 3. Transaction Safety Implementation

**Before:**
```typescript
// src/server/api/routers/project.ts
deleteApiKey: sensitiveProcedure
  .input(z.object({ apiKeyId: z.string(), reason: z.string() }))
  .mutation(async ({ ctx, input }) => {
    // ⚠️ Not atomic
    await db.auditLog.create({ ... });
    await db.apiKey.delete({ ... });
  }),
```

**After:**
```typescript
// src/server/api/routers/project.ts
import { ERROR_MESSAGES } from "@/lib/error-messages";

deleteApiKey: sensitiveProcedure
  .input(
    z.object({
      apiKeyId: z.string(),
      reason: z
        .string()
        .min(1, ERROR_MESSAGES.API_KEY_DELETION_REASON_REQUIRED)
        .max(500, ERROR_MESSAGES.VALIDATION_REASON_TOO_LONG),
    })
  )
  .mutation(async ({ ctx, input }) => {
    const userId = ctx.session.user.id;

    try {
      // Fetch API key first (for validation and audit metadata)
      const apiKey = await db.apiKey.findUnique({
        where: { id: input.apiKeyId },
        include: { project: { select: { id: true, teamId: true } } },
      });

      if (!apiKey) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: ERROR_MESSAGES.API_KEY_NOT_FOUND,
        });
      }

      // Permission check
      await ensureProjectAccess(userId, apiKey.project.id);

      // ✅ Atomic transaction: Audit log + Delete
      await db.$transaction([
        db.auditLog.create({
          data: {
            userId,
            actionType: "api_key_deleted",
            resourceType: "api_key",
            resourceId: input.apiKeyId,
            metadata: {
              reason: input.reason,
              projectId: apiKey.project.id,
              provider: apiKey.provider,
              wasActive: apiKey.isActive,
            },
          },
        }),
        db.apiKey.delete({
          where: { id: input.apiKeyId },
        }),
      ]);

      return { success: true };
    } catch (error) {
      // Improved error logging
      console.error("[deleteApiKey] Error:", {
        userId,
        apiKeyId: input.apiKeyId,
        error: error instanceof Error ? error.message : "Unknown error",
      });

      throw error;
    }
  }),
```

**Apply same pattern to:**
- `disableApiKey`
- `enableApiKey`
- Any other critical operations

### 4. Frontend Validation (Immediate Feedback)

```typescript
// src/components/dialogs/ConfirmDeleteKeyDialog.tsx
import { zodResolver } from "@hookform/resolvers/zod";
import { useForm } from "react-hook-form";
import { z } from "zod";

const deleteKeySchema = z.object({
  confirmText: z.literal("삭제", {
    errorMap: () => ({ message: "'삭제'를 정확히 입력해주세요" }),
  }),
  reason: z
    .string()
    .min(1, "삭제 사유는 필수입니다")
    .max(500, "사유는 500자 이내로 입력해주세요"),
});

type DeleteKeyForm = z.infer<typeof deleteKeySchema>;

export function ConfirmDeleteKeyDialog({ ... }) {
  const { register, handleSubmit, formState: { errors } } = useForm<DeleteKeyForm>({
    resolver: zodResolver(deleteKeySchema),
  });

  const onSubmit = (data: DeleteKeyForm) => {
    onConfirm(data.reason);
  };

  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Textarea
        {...register("reason")}
        placeholder="예: 더 이상 사용하지 않음"
      />
      {errors.reason && (
        <p className="text-destructive text-sm">{errors.reason.message}</p>
      )}
      {/* ... */}
    </form>
  );
}
```

### 5. Error Logging 표준화

```typescript
// src/lib/logger.ts
type LogLevel = "info" | "warn" | "error";

interface LogContext {
  userId?: string;
  action?: string;
  resource?: string;
  [key: string]: unknown;
}

export function logError(message: string, context?: LogContext) {
  const timestamp = new Date().toISOString();
  const logEntry = {
    timestamp,
    level: "error" as LogLevel,
    message,
    ...context,
  };

  if (process.env.NODE_ENV === "development") {
    console.error("[ERROR]", logEntry);
  } else {
    // Production: Send to Sentry, Datadog, etc.
    // Sentry.captureException(new Error(message), { extra: context });
  }
}
```

**Usage in mutations:**
```typescript
// src/server/api/routers/project.ts
import { logError } from "@/lib/logger";

deleteApiKey: sensitiveProcedure
  .mutation(async ({ ctx, input }) => {
    try {
      // ... implementation
    } catch (error) {
      logError("API key deletion failed", {
        userId: ctx.session.user.id,
        action: "deleteApiKey",
        resource: input.apiKeyId,
        error: error instanceof Error ? error.message : "Unknown error",
      });

      throw error;
    }
  }),
```

---

## Complete File Changes

### Files to Update

1. **Create:** `src/lib/error-messages.ts` (new file)
2. **Create:** `src/lib/logger.ts` (new file)
3. **Update:** `src/server/api/routers/team.ts` (5 error messages)
4. **Update:** `src/server/api/routers/project.ts` (12 error messages + transactions)
5. **Update:** `src/components/dialogs/ConfirmDeleteKeyDialog.tsx` (validation)
6. **Update:** `src/components/dialogs/ConfirmDisableKeyDialog.tsx` (validation)
7. **Update:** `src/components/dialogs/AddApiKeyDialog.tsx` (validation)

---

## Testing Checklist

### Error Message Tests
- [ ] 팀 접근 거부 시 "이 팀에 접근 권한이 없습니다" 표시
- [ ] 프로젝트 접근 거부 시 "이 프로젝트에 접근 권한이 없습니다" 표시
- [ ] API 키 삭제 사유 없이 제출 시 "삭제 사유는 필수입니다" 표시
- [ ] 500자 초과 입력 시 "사유는 500자 이내로 입력해주세요" 표시

### Transaction Tests
- [ ] API 키 삭제 중 오류 발생 시 audit log도 롤백되는지 확인
- [ ] Transaction 실패 시 명확한 한국어 에러 메시지 표시
- [ ] Database consistency: Delete 실패 시 audit log 없음

### Validation Tests
- [ ] 501자 사유 입력 시 프론트엔드에서 차단
- [ ] 빈 사유 입력 시 프론트엔드에서 차단
- [ ] "삭제" 외 다른 텍스트 입력 시 제출 버튼 비활성화

---

## Success Metrics

- [ ] 100% 한국어 에러 메시지 (0개 영어 메시지 남음)
- [ ] 모든 critical operations에 transaction 적용
- [ ] Input validation 100% 적용
- [ ] 에러 로그 표준화 100% 완료

---

## Related Stories

- **Story 1.10**: 프로젝트 멤버 및 API 키 관리 UI (기반)
- **Story 1.11**: 보안 강화 (input validation과 연계)
- **Story 1.12**: 성능 최적화 (transaction 성능 고려)
